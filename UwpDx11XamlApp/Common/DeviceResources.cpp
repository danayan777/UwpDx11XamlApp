...
void DX::DeviceResources::CreateWindowSizeDependentResources() 
{
	...
	else
	{
		...
		// XAML 相互運用機能の使用時に、合成用にスワップ チェーンを作成する必要があります。
...
void DX::DeviceResources::UpdateRenderTargetSize()
{
	...
	if (!DisplayMetrics::SupportHighResolutions && m_dpi > DisplayMetrics::DpiThreshold)
	{
		...
		// デバイスが縦方向の場合、高さ > 幅です。大きいディメンションを
		//幅のしきい値と比較し、小さいディメンションを
		//高さのしきい値と比較します。
	...
	// 必要なレンダー ターゲットのサイズをピクセル単位で計算します。
...
// このメソッドは、XMAL コントロールが作成 (または再作成) されたときに呼び出されます。
...
// このメソッドは、CompositionScaleChanged イベントのイベント ハンドラーで呼び出されます。
...
void DX::DeviceResources::Present() 
{
	...
	// 上書きされます。dirty rect または scroll rect を使用する場合は、この呼び出しを変更する必要があります。
